...
Final model snapshot pickle file is stored in the google drive (https://drive.google.com/drive/folders/1BFsXMFJwrPmru0ctgIkG0VoYPFddMTTH?usp=sharing).
...
The dataset we used for our project was gathered by students and faculties from RISD. It is composed by pictures of three different types of Mexican potteries. Currently, there are 432 pictures for Cazuela, 298 pictures for Copalero, and 471 pictures for Tinaja.

### Preprocess

We wrote a [preprocess.py](./preprocess.py) to help us preprocess the raw images we got.

## Results

#### Qualitative Results:

Below is a snapshot of the training images we used for training the StyleGAN model:

![real_image](./results/reals.png)

The following is a snapshot of the pottery images generated by our model:

![generated_images](./results/fakes009726.png)

#### Quantitative Results:

We got an FID score of 90.8457 for our final model.

Subject metrics:

Ratio of unique images generated

Even though our model is able to generate high-quality pottery images, we also noticed that there are some duplicated images generated by our model. In order to find the ratio of unique images generated, we first need a criterion for considering one image as a duplication of another. We had several ideas of doing this and finally decided to first flatten the images into a one-dimensional vector and then compare the images directly by calculating their cosine similarity. The reason we choose this method instead of other machine learning methods like clustering or embedding is that we are trying to find identical images here. Therefore, it’s more efficient to compare the images themselves directly than some machine-learned representation of them. 

As for the threshold of cosine similarity for considering two images as identical, we tested a few values from 0.9 to 1 and observed the images that were considered identical under those criteria. We then decided to choose the threshold of 0.99 of cosine similarity for considering two images as identical.

With the above criterion, we did a Monte Carlo Simulation where we generated 100 images each time and repeated the procedure for 100 times in order to calculate the ratio of unique images generated. As a result, the ratio of unique images generated by our model has a mean of 0.61 with a standard deviation of 0.04. This means that around 60 percent of the generated images are unique while the other 40 percentage are just duplication of one another.


Novelty of generated images

Apart from generating duplicated images, we also noticed that our model was generating images that looks almost identical to the training images. While this is fine with normal GANs which try to generate images that look real, it does not satisfy the need of this project which is to generate new Mexican pottery images that we can make into production. Therefore, we would want to evaluate the novelty of the images generated by our model.

With the same criterion for considering two images as identical and same Monte Carlo Simulation procedure as described above, we found that the ratio of novel images generated by our model has a mean of 0.47 and a standard deviation of 0.05. This means that one out of two generated images is a novel Mexican pottery images generated by our model.

More information and codes of model results are stored in [results.ipynb](./results.ipynb).
Partial results during the training of the model are store in [results](./results) folder.

# Here's the original post from the developers of styleGAN:
